    ...
    >
      ...
      {horizontalReference !== undefined && horizontalReference >= yRange.min && horizontalReference <= yRange.max && (
        <line
          ...
          x1={MARGIN.left}
          y1={scaleY(horizontalReference, yRange)}
          x2={MARGIN.left + INNER_WIDTH}
          y2={scaleY(horizontalReference, yRange)}
        />
      )}
...
function PlotCard({ eyebrow, title, note, children, className }: { eyebrow: string; title: string; note: string; children: ReactNode; className?: string }) {
  return (
...
export default function StatePlots({ trajectory, currentPoint }: StatePlotsProps) {
  ...
  const phasePaths = wrappedSegments.map((segment, index) => ({
    key: `phase-${index}`,
    d: pathFromPoints(segment.map((point) => ({ x: point.phiDegrees, y: point.rOffset })), phiRange, rOffsetRange),
  }))
  ...
  return (
    <section className="state-plot-grid" aria-label="Guiding-center state plots">
      ...
      <PlotCard eyebrow="Reduced phase space" title="φ vs r − 1" note="Same wrapped φ convention; dashed line marks corotation" className="phase-space-card">
        <PlotFrame
